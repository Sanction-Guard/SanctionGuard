...
describe('Search Controller', () => {
  ...
  describe('search', () => {
    test('should perform search and create audit log', async () => {
      ...
      const mockResults = ['result1', 'result2'];
      searchService.performSearch.mockResolvedValueOnce(mockResults);
      AuditLog.prototype.save = jest.fn().mockResolvedValue(true);
    ...
    test('should handle errors', async () => {
      ...
      // Assert
      expect(mockResponse.status).toHaveBeenCalledWith(500);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Search failed' });
    });
  });
  ...
  describe('getDatabaseStatus', () => {
    ...
    test('should handle errors', async () => {
      ...
      // Assert
      expect(mockResponse.status).toHaveBeenCalledWith(500);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Status check failed' });
    });
  });
  ...
  describe('getAuditLogs', () => {
    ...
    test('should handle errors', async () => {
      ...
      // Assert
      expect(mockResponse.status).toHaveBeenCalledWith(500);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Failed to fetch logs' });
    });
  });
});